...
const connectSessionFirebase = function connectSessionFirebase({ Store }) {
  ...
  FirebaseStore.prototype.getAll = async function getAll(fn) {
    let answer = [];
    await this.database.ref(this.sessions)
      .once('value', (snapshot) => {
        answer.push(snapshot.toJSON())
    })
    return answer[0]
  }

  FirebaseStore.prototype.getAllUsers = async function getAll(fn) {
    let answer = [];
    const now = Date.now()
    await this.database.ref(this.sessions)
      .once('value', (snapshot) => {
        snapshot.forEach((v) => {
          answer.push(JSON.parse(v.val().sess).user)
        })
